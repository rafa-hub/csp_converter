...
class MiSolverPrintCallbacks : public  XCSP3PrintCallbacks{
    ...
    private:
        ...
        bool necesito_crear_matriz=TRUE;    // Variable para identificar cuándo crear la matriz
        int dimension_matriz=0;             //Guarda la dimensión definitiva de la matriz creada
        
        vector <XVariable> valores;      //Guarda la lista de valores
    ...
    void escribe_fichero_csp(){
        ...
        fichero_salida << "x " << datos_csp.size() << endl;
                
        for (unsigned int j=0; j<datos_csp.size();j++)
        {
            cout << "v " << (j+1) << " " << datos_csp[j] << endl;
            fichero_salida << "v " << (j+1) << " " << datos_csp[j] << endl;
...
=========Comienzo de las funciones que invoca el parser ===================================== */
    ...
    //#ifdef midebug
        for(itero=lista_arrays.begin();itero!=lista_arrays.end();itero++)
        {
            ...
            cout << "base - " << base_array[*itero] << endl;
            cout << "rango valores - " << rango_variable[*itero] << endl;
